...
import * as mask from './index';
import {stubbedImageVals} from './test_util';

describeWithFlags('FaceMask', NODE_ENVS, () => {
  let model: FaceMaskModel;
  
  beforeAll(async () => {
    // originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    // jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000000;
    model = await mask.loadMask();
  ...
  it('infer returns objects with expected properties', async () => {
    ...
    const input: tf.Tensor3D = tf.tensor3d(stubbedImageVals.slice(0, 37632), [224, 224, 3]);
  ...
  it('model infer doen\'t leak', async () => {
    const x: tf.Tensor3D = tf.zeros([224, 224, 3]);
